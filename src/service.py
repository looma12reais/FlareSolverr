...
def _resolve_challenge(req: V1RequestBase, method: str) -> ChallengeResolutionT:
    timeout = int(req.maxTimeout or 0) / 1000
    driver = None
    try:
        if req.session:
            session_id = req.session
        ...
        return cast(
            ChallengeResolutionT,
            func_timeout(timeout, _evil_logic, (req, driver, method)),
        )
...
def _evil_logic(
    ...
    if req.url is None:
        logging.info("req.url is None. shouldn't happen")
...
def _post_request(req: V1RequestBase, driver: WebDriver):
    ...
    query_string = (
        req.postData
        if req.postData and req.postData[0] != "?"
        else req.postData[1:]
        if req.postData
        else ""
    )
